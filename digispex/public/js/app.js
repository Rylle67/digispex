/* ---- API data caches ---- */
let _customProductsCache = [];
let _customImagesCache   = {};
let _customDescsCache    = {};
let _hiddenProductsCache = [];


/* Stock loaded from API into memory on init */
let _stockCache = {};
function getStockStatus(id) {
  return (_stockCache[id]?.status) || 'normal';
}
let currentFilter = 'All';
let currentSlotKey = null;

/* Async initialization — replaces synchronous DB.init() */
async function appInit() {
  // Load stock, custom products, and images from API
  const [stockData, customProds, imagesData, hiddenIds] = await Promise.all([
    DS_API.admin.getStock().catch(() => ({})),
    DS_API.admin.getProducts().catch(() => []),
    DS_API.admin.getImages().catch(() => ({})),
    DS_API.admin.getHidden().catch(() => []),
  ]);
  _stockCache = stockData;
  _customProductsCache = customProds;
  _customImagesCache = imagesData;
  _hiddenProductsCache = hiddenIds;

  // Preload user data
  if (DS_API.auth.isLoggedIn()) {
    await Promise.all([
      DS_API.cart.get().catch(() => []),
      DS_API.build.get().catch(() => ({})),
      DS_API.wishlist.get().catch(() => []),
    ]);
    // Sync wishlist into features.js cache
    if (typeof _loadWishlist === 'function') await _loadWishlist();
  }
...
// Run after DOM ready (called from HTML)
...
function renderNav() {
  const _cachedCart = DS_API.cart._cartCache || [];
  document.getElementById('cartCount').textContent = _cachedCart.reduce((s,i)=>s+i.qty,0);
...
window.addEventListener('storage', function(e) {
  // Legacy: no-op. Data is now managed server-side.
  if (false && e.key === 'ds_custom_products') {
...
function renderStore() {
  ...
  const _cp = _customProductsCache || [];
...
async function addToCart(productId) {
  ...
  if (typeof INVENTORY !== 'undefined') {
    const cart    = await DS_API.cart.get();
  ...
  await DS_API.cart.add(productId);
  renderNav();
...
async function updateQty(productId, delta) {
  ...
  await DS_API.cart.update(productId, delta);
  renderCart();
  renderNav();
}

async function removeFromCart(productId) {
  await DS_API.cart.remove(productId);
  renderCart();
  renderNav();
}
...
function openCheckout() {
  if (!(await DS_API.cart.count())) return;
...
async function placeOrder(e) {
  ...
  await DS_API.orders.place(order);
...
function renderOrders() {
  const orders = await DS_API.orders.get();
...
async function renderBuilder() {
  const build    = await DS_API.build.get();
  const platform = getCPUPlatform(build);
  const _customImgs = _customImagesCache || {};
  let total = 0, count = 0;
...
async function openSlotModal(slotKey) {
  ...
  const _slotImgs = _customImagesCache || {};
...
function addBuilderToCart() {
  ...
  const ids = Object.values(await DS_API.build.get()).filter(Boolean);
  if (!ids.length) { showToast('No parts selected', 'error'); return; }
  for (const id of ids) await DS_API.cart.add(id);
  renderNav();
...
function openPaymentReview() {
  var cart = await DS_API.cart.get();
