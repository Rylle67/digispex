...
/* Wishlist is now served from PostgreSQL via DS_API */
let _wishlist = [];

/* Load wishlist from server on init */
async function _loadWishlist() {
  if (DS_API.auth.isLoggedIn()) {
    _wishlist = await DS_API.wishlist.get();
  } else {
    _wishlist = [];
  }
}
...
async function toggleWishlist(productId, btn) {
  ...
  try {
    const result = await DS_API.wishlist.toggle(productId);
    _wishlist = result.ids;
    if (result.added) {
