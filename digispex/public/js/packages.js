/* ============================================================
   DigiSpex Online Shop — Package Customization Engine
   packages.js  v1.0
...
/* Packages are loaded from API into _packagesCache. Falls back to PACKAGES. */
function _getPackages() {
  return _packagesCache || PACKAGES;
}
...
function openPkgSwap(slot, pkgId) {
  ...
  const _swapCustom = (typeof _customProductsCache !== 'undefined' ? _customProductsCache : []);
...
function renderLaptops() {
  ...
  const _customProds = (typeof _customProductsCache !== 'undefined' ? _customProductsCache : []);
  ...
  el.innerHTML = items.length
    ? items.map(p => {
        ...
        const _customImgs = (typeof _customImagesCache !== 'undefined' ? _customImagesCache : {});
...
function openLaptopDetail(pid) {
  ...
  const _dImgs = (typeof _customImagesCache !== 'undefined' ? _customImagesCache : {});
